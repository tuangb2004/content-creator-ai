      ...
      <div className="relative z-10 h-full flex flex-col justify-center items-center text-center px-6">
        <div className="animate-fade-in-up w-full max-w-4xl">
          <span className="block text-xs md:text-sm font-medium uppercase tracking-[0.2em] text-white/90 mb-6 backdrop-blur-sm bg-white/5 px-4 py-2 rounded-full mx-auto w-fit border border-white/10">
            {t?.landing?.hero?.badge || 'AI-Powered Content Suite'}
          </span>
          <h1 className="text-6xl md:text-8xl lg:text-9xl font-serif font-normal text-white tracking-tight mb-8 drop-shadow-xl">
            {t?.landing?.hero?.headline_1 || 'Create'} <span className="italic text-[#F5F2EB]">{t?.landing?.hero?.headline_2 || 'everything.'}</span>
          </h1>
          <p className="max-w-xl mx-auto text-lg md:text-xl text-white/90 font-light leading-relaxed mb-12 text-shadow-sm whitespace-pre-wrap">
            {t?.landing?.hero?.subheadline || 'From viral tweets to SEO articles.\nCreatorAI creates tailored content for your brand in seconds.'}
          </p>
          
          <button 
            onClick={onGetStarted}
            className="group relative px-10 py-4 bg-[#F5F2EB] text-[#2C2A26] rounded-full text-sm font-semibold uppercase tracking-widest hover:bg-white transition-all duration-500 overflow-hidden shadow-2xl hover:shadow-white/20 inline-block cursor-pointer"
          >
            <span className="relative z-10 group-hover:text-[#2C2A26]">{t?.landing?.hero?.cta || 'Start Creating'}</span>
          </button>
        </div>
      </div>
