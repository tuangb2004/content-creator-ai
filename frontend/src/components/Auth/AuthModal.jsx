...
const AuthModal = ({ isOpen, onClose, onLogin, onNavigate, type = 'signup', onSwitchType, forceVerifyMode = false, verificationCode = null }) => {
  ...
  // Effect to handle verification code from URL
  useEffect(() => {
  ...
  const handleEmailVerification = async (code) => {
    ...
    try {
      ...
      // Auto-close after 2 seconds and switch to sign-in
      setTimeout(() => {
        setVerificationStatus(null);
        setMode('email');
        if (onSwitchType) onSwitchType('signin');
      }, 2000);
    } catch (error) {
  ...
  const handleBackToLanding = async () => {
    try {
      // Close modal immediately to prevent flash of empty state
      onClose();
      // Sign out user
      await signOut(auth);
      // Navigate is not needed since we're already on landing page
  ...
  const handleEmailSubmit = async (e) => {
    ...
    try {
      if (type === 'signup') {
        ...
        // NEW FLOW: Giữ user ở landing page, chuyển modal sang verify mode
        // Không redirect đến dashboard để tránh page reload
        if (emailSent) {
          toast.success(t?.auth?.accountCreated || 'Account created! Please check your email to verify your account.');
          // Switch to verify email mode - KEEP MODAL OPEN on landing page
          setMode('verify_email');
          // Do NOT navigate to dashboard - stay on landing page
        } else {
        ...
        try {
          ...
          // NEW FLOW: Check if email is verified
          // If not, switch to verify mode instead of redirecting
          if (userCredential?.user && !userCredential.user.emailVerified) {
            // User exists but email not verified - show verify screen on landing page
            toast.info(t?.auth?.pleaseVerifyEmail || 'Please verify your email to continue.');
            setMode('verify_email');
            // Do NOT close modal or navigate - stay on landing page
          } else {
            // Email verified or social login - proceed to dashboard
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
      ...
      <div className="relative bg-[#F5F2EB] dark:bg-[#1C1B19] w-full max-w-md shadow-2xl overflow-hidden border border-[#D6D1C7] dark:border-[#433E38] animate-fade-in-up rounded-sm transition-all duration-300">
        ...
        <div className="p-8 md:p-10 text-center">
          ...
          {/* VIEW: VERIFY EMAIL MODE (Blocking Screen or Verification from Link) */}
          {mode === 'verify_email' && (
            <>
              {/* Processing State - Verifying from link */}
              {verificationStatus === 'processing' && (
                <div className="text-center animate-fade-in-up">
                  {/* Spinner */}
                  <div className={`w-16 h-16 mx-auto mb-6 border-4 border-t-transparent rounded-full animate-spin ${theme === 'dark' ? 'border-[#F5F2EB]' : 'border-[#2C2A26]'
                    }`}></div>
                  ...
                  {/* Message */}
                  <p className="text-[#5D5A53] dark:text-[#A8A29E] font-light text-sm">
              ...
              {verificationStatus === 'success' && (
                <div className="text-center animate-fade-in-up">
                  {/* Success Icon */}
                  <div className="w-20 h-20 bg-green-500 dark:bg-green-600 rounded-full flex items-center justify-center mx-auto mb-6 shadow-xl">
                  ...
                  {/* Message */}
                  <p className="text-[#5D5A53] dark:text-[#A8A29E] font-light text-sm">
                    {t?.auth?.redirectingToSignIn || 'Redirecting to sign in...'}
                  </p>
                </div>
              )}
              ...
              {verificationStatus === 'error' && (
                <div className="text-center animate-fade-in-up">
                  {/* Error Icon */}
                  <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 shadow-xl ${theme === 'dark' ? 'bg-red-900/20 text-red-400' : 'bg-red-100 text-red-600'
                    }`}>
                  ...
                  {/* Error Message */}
                  <p className="text-[#5D5A53] dark:text-[#A8A29E] font-light text-sm mb-8">
                    {verificationError}
                  </p>
              ...
              {/* Manual Verification Check UI - When user is logged in but not verified */}
              {!verificationStatus && user && !user.emailVerified && (
                <>
                  {/* Large Email Icon */}
          ...
          {!showWaitingScreen && mode !== 'verify_email' && (
            <>
              {/* Header Text */}
              <div className="mb-8">
                <h2 className={`text-3xl font-serif mb-2 transition-colors duration-300 ${theme === 'dark' ? 'text-[#F5F2EB]' : 'text-[#2C2A26]'
                  }`}>
                ...
                <p className={`font-light text-sm transition-colors duration-300 ${theme === 'dark' ? 'text-[#A8A29E]' : 'text-[#5D5A53]'
                  }`}>
                  {mode === 'forgot_password'
                ...
                <div className="space-y-3 animate-fade-in-up">
                  ...
                  <div className="relative my-6">
                    <div className="absolute inset-0 flex items-center">
                      <div className={`w-full border-t transition-colors duration-300 ${theme === 'dark' ? 'border-[#433E38]' : 'border-[#D6D1C7]'
                        }`}></div>
                    </div>
                    <div className="relative flex justify-center text-xs uppercase tracking-widest">
                      <span className={`px-2 transition-colors duration-300 ${theme === 'dark'
                  ...
                  <button
                    ...
                    className={`w-full py-3 text-xs font-medium uppercase tracking-widest transition-colors rounded-sm ${theme === 'dark'
                  ...
                  <div className="pt-4">
                    {type === 'signin' ? (
                      <p className={`text-xs transition-colors duration-300 ${theme === 'dark' ? 'text-[#A8A29E]' : 'text-[#5D5A53]'
                        }`}>
                        {t?.auth?.dontHaveAccount || "Don't have an account?"} <button onClick={() => switchAuth('signup')} className={`font-bold transition-colors duration-300 hover:underline ${theme === 'dark' ? 'text-[#F5F2EB]' : 'text-[#2C2A26]'
                          }`}>{t?.auth?.signUp || 'Sign Up'}</button>
                      </p>
                    ) : (
                      <p className={`text-xs transition-colors duration-300 ${theme === 'dark' ? 'text-[#A8A29E]' : 'text-[#5D5A53]'
                        }`}>
                        {t?.auth?.alreadyHaveAccount || 'Already have an account?'} <button onClick={() => switchAuth('signin')} className={`font-bold transition-colors duration-300 hover:underline ${theme === 'dark' ? 'text-[#F5F2EB]' : 'text-[#2C2A26]'
                    ...
                    <>
                      <div className="relative">
                        <input
                          ...
                          className={`w-full px-4 py-3 outline-none transition-colors text-sm rounded-sm ${theme === 'dark'
                          ...
                          required
                        />
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`w-5 h-5 absolute right-3 top-1/2 -translate-y-1/2 ${theme === 'dark' ? 'text-[#A8A29E]' : 'text-[#A8A29E]'
                          }`}>
                      ...
                      <button
                        type="submit"
                        className={`w-full py-3 text-xs font-medium uppercase tracking-widest transition-colors rounded-sm mt-2 ${theme === 'dark'
                          ? 'bg-[#F5F2EB] text-[#2C2A26] hover:bg-[#EBE7DE]'
                          : 'bg-[#2C2A26] text-[#F5F2EB] hover:bg-[#433E38]'
                          }`}
                      >
              ...
              {showVerificationMessage && mode === 'email' && (
                <div className={`border p-6 rounded-sm animate-fade-in-up text-left ${theme === 'dark'
                  ? 'bg-[#2C2A26] border-[#433E38]'
                  : 'bg-[#EBE7DE] border-[#D6D1C7]'
                  }`}>
                  <div className="flex items-start gap-3 mb-4">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`w-6 h-6 flex-shrink-0 mt-0.5 ${theme === 'dark' ? 'text-[#A8A29E]' : 'text-[#5D5A53]'
                      }`}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
                    </svg>
                    <div className="flex-1 min-w-0">
                      <h3 className={`font-semibold text-sm mb-2 ${theme === 'dark' ? 'text-[#F5F2EB]' : 'text-[#2C2A26]'
                        }`}>
                        {t?.auth?.verifyEmail || 'Verify your email address'}
                      </h3>
                      <p className={`text-xs mb-2 leading-relaxed ${theme === 'dark' ? 'text-[#A8A29E]' : 'text-[#5D5A53]'
                        }`}>
                      ...
                      <p className={`text-xs mb-0 leading-relaxed ${theme === 'dark' ? 'text-[#A8A29E]' : 'text-[#5D5A53]'
                        }`}>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className={`flex-1 px-4 py-2.5 text-xs font-medium transition-colors rounded-sm ${theme === 'dark'
                    ...
                    <button
                      ...
                      className={`flex-1 px-4 py-2.5 text-xs font-medium transition-colors border rounded-sm ${theme === 'dark'
              ...
              {mode === 'email' && !showVerificationMessage && (
                <form className="space-y-4 animate-fade-in-up" onSubmit={handleEmailSubmit}>
                  ...
                  <div>
                    <div className="relative">
                      <input
                        ...
                        onChange={handleInputChange}
                        className={`w-full px-4 py-3 pr-10 outline-none transition-colors text-sm rounded-sm ${theme === 'dark'
                          ? 'bg-[#2C2A26] border border-[#433E38] text-[#F5F2EB] placeholder-[#A8A29E] focus:border-[#F5F2EB]'
                          : 'bg-white border border-[#D6D1C7] text-[#2C2A26] placeholder-[#A8A29E] focus:border-[#2C2A26]'
                          }`}
                      />
                      <button
                        ...
                        className={`absolute right-3 top-1/2 -translate-y-1/2 transition-colors p-1 rounded hover:bg-opacity-10 ${theme === 'dark'
                    ...
                    {type === 'signup' && formData.password && (
                      <div className="mt-2 space-y-1">
                        <div className="flex items-center gap-2">
                          <div className={`flex-1 h-1.5 rounded-full overflow-hidden ${theme === 'dark' ? 'bg-[#433E38]' : 'bg-[#EBE7DE]'
                            }`}>
                            <div
                              className={`h-full transition-all duration-300 ${passwordStrength.strength === 'strong'
                          ...
                          <span className={`text-xs font-medium ${theme === 'dark' ? 'text-[#A8A29E]' : 'text-[#5D5A53]'
                            }`}>
                            {passwordStrength.strength.toUpperCase()}
                          </span>
                        </div>
                        {passwordStrength.feedback.length > 0 && (
                          <div className={`text-xs space-y-0.5 ${theme === 'dark' ? 'text-[#A8A29E]' : 'text-[#5D5A53]'
                            }`}>
                            {passwordRequirements.map(req => {
                              const met = req.test(formData.password);
                              return (
                                <div key={req.id} className={`flex items-center gap-1.5 ${met
                      ...
                      <div className="flex justify-between items-center mt-2">
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input
                            ...
                            className={`w-4 h-4 rounded transition-colors ${theme === 'dark'
                          ...
                          />
                          <span className={`text-xs transition-colors ${theme === 'dark' ? 'text-[#A8A29E]' : 'text-[#5D5A53]'
                            }`}>
                        ...
                        <button
                          ...
                          className={`text-xs font-medium transition-colors hover:underline ${theme === 'dark' ? 'text-[#A8A29E] hover:text-[#F5F2EB]' : 'text-[#5D5A53] hover:text-[#2C2A26]'
                            }`}
                        >
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className={`w-full py-3 text-xs font-medium uppercase tracking-widest transition-colors disabled:opacity-50 rounded-sm mt-2 ${theme === 'dark'
                      ? 'bg-[#F5F2EB] text-[#2C2A26] hover:bg-[#EBE7DE]'
                      : 'bg-[#2C2A26] text-[#F5F2EB] hover:bg-[#433E38]'
                      }`}
                  >
                  ...
                  <div className="pt-4">
                    {type === 'signin' ? (
                      <p className={`text-xs transition-colors duration-300 ${theme === 'dark' ? 'text-[#A8A29E]' : 'text-[#5D5A53]'
                        ...
                        {t?.auth?.newHere || 'New here?'} <button type="button" onClick={() => switchAuth('signup')} className={`font-bold transition-colors duration-300 hover:underline ${theme === 'dark' ? 'text-[#F5F2EB]' : 'text-[#2C2A26]'
                          }`}>{t?.auth?.createAnAccount || 'Create an account'}</button>
                      </p>
                    ) : (
                      <p className={`text-xs transition-colors duration-300 ${theme === 'dark' ? 'text-[#A8A29E]' : 'text-[#5D5A53]'
                        }`}>
                        {t?.auth?.memberAlready || 'Member already?'} <button type="button" onClick={() => switchAuth('signin')} className={`font-bold transition-colors duration-300 hover:underline ${theme === 'dark' ? 'text-[#F5F2EB]' : 'text-[#2C2A26]'
              ...
              {mode !== 'forgot_password' && (
                <p className={`mt-8 text-xs font-light transition-colors duration-300 ${theme === 'dark' ? 'text-[#A8A29E]' : 'text-[#A8A29E]'
                  }`}>
                  {t?.auth?.agreeTerms || 'By clicking continue, you agree to our'} <button
                    type="button"
                    onClick={(e) => handleLegalNav(e, 'terms')}
                    className={`underline transition-colors duration-300 hover:underline ${theme === 'dark' ? 'hover:text-[#F5F2EB]' : 'hover:text-[#2C2A26]'
                      }`}
...
const SocialButton = ({ icon, label, onClick }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full flex items-center justify-center gap-3 py-3 transition-colors group rounded-sm ${theme === 'dark'
    ...
    >
      ...
      <span className={`text-sm font-medium transition-colors ${theme === 'dark'
