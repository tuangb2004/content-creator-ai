...
export const payosWebhook = functions.https.onRequest(async (req, res) => {
  ...
  if (code === '00') {
    try {
      ...
      const creditAmount = paymentLinkData.creditAmount; // Check if this is a credit purchase

      // If creditAmount exists, this is a credit purchase (not plan upgrade)
      if (creditAmount) {
        console.log(`[payosWebhook] Processing credit purchase: ${creditAmount} credits for user ${userId}`);

        // Only add credits, don't update plan
        await setCredits(userId, creditAmount, {
        ...
        // Plan upgrade: Update plan and set credits
        console.log(`[payosWebhook] Processing plan upgrade: ${planName} for user ${userId}`);
